...
public class LassoFitGenerator {
    ...
    public void setFeatureValues(int idx, double[] values) {
        for (int i = 0; i < values.length; i++) {
            observations[idx][i] = values[i];
        }
    }

    public double[] getFeatureValues(int idx) {
    ...
    private LassoFit getLassoFit(int maxAllowedFeaturesPerModel) {
        ...
        for (int j = 0; j < numFeatures; j++) {
            ...
            for (int i = 0; i < numObservations; i++) {
                observations[j][i] = (double) (factor * (observations[j][i] - mean));
            }
    ...
    public void setTargets(double[] targets) {
        for (int i = 0; i < numObservations; i++) {
            this.targets[i] = (double) targets[i];
        }
    }

    public void setTarget(int idx, double target) {
        this.targets[idx] = (double) target;
    }
